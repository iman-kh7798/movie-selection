...
const TMDB_KEY = process.env.NEXT_PUBLIC_TMDB_KEY!; // set in .env
console.log(TMDB_KEY);
// Build a discover query based on top traits
export async function discoverByTraits(
  scores: Record<Trait, number>
): Promise<TMDbMovie[]> {
  const entries = Object.entries(scores) as [Trait, number][];
  entries.sort((a, b) => b[1] - a[1]);

  const top = entries.slice(0, 2).map(([t]) => t); // top-2 traits
  const genreIds = top.flatMap((t) => TRAIT_TO_TMDB_GENRES[t] ?? []);

  const url = new URL(`${TMDB_BASE}/discover/movie`);
  url.searchParams.set("api_key", TMDB_KEY);
  url.searchParams.set("include_adult", "false");
  url.searchParams.set("language", "fa-IR"); // Persian titles where available
  url.searchParams.set("sort_by", "vote_average.desc");
  url.searchParams.set("vote_count.gte", "200"); // avoid obscure titles
